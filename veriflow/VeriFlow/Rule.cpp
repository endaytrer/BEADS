...
EquivalenceClass Rule::getEquivalenceClass() const {
    ...
    for (int i = 0; i < ALL_FIELD_INDEX_END_MARKER; i++) {
        ...
        if ((i == DL_SRC) || (i == DL_DST)) { //NOTE: set equivalent class values. if MAC, translate MAC; if IP, translate IP; else, convert directly to integer
            ...
            fieldValue = strtoul(this->fieldValue[i].c_str(), nullptr, 10);
            fieldMask = strtoul(this->fieldMask[i].c_str(), nullptr, 10);
        }

        uint64_t maskedFieldValue = fieldValue & fieldMask;

        for (unsigned int j = 0; j < ::fieldWidth[i]; j++) {
            rulePacketClass.lowerBound[i] <<= 1; // setting from MSB to LSB
            rulePacketClass.upperBound[i] <<= 1;

            uint64_t maskBit = (uint64_t) 1 << ((unsigned int) (::fieldWidth[i] - 1) - j); // bit to test j == 0 => msb, j == fieldWidth - 1 => lsb
            if ((fieldMask & maskBit) == 0) // wildcard bit
            {
                rulePacketClass.upperBound[i] |= 1; // lower bound = 0, upper bound = 1;
            } else {
...
bool Rule::equals(const Rule &other) const {
    for (int i = 0; i < ALL_FIELD_INDEX_END_MARKER; i++) { // compare mask.
    ...
    if ((this->type == other.type) // compare rule itself
        && (this->wildcards == other.wildcards)
...
int Rule::operator()() const { //NOTE: hashing, I guess
    int retVal = 0;
    for (int i = 0; i < ALL_FIELD_INDEX_END_MARKER; i++) {
...
string Rule::toString() const {
    char buffer[1024];
    sprintf(buffer,
            "[Rule] type: %d, dlSrcAddr: %s, dlSrcAddrMask: %s, dlDstAddr: %s, dlDstAddrMask: %s, nwSrcAddr: %s, nwSrcAddrMask: %s, nwDstAddr: %s, nwDstAddrMask: %s, location: %s, nextHop: %s, in_port: %u, priority: %u",
            this->type,
    ...
    sprintf(buffer,
            "wildcards: %u, in_port: %s, dl_type: %s, dl_vlan: %s, dl_vlan_pcp: %s, mpls_label: %s, mpls_tc: %s, nw_proto: %s, nw_tos: %s, tp_src: %s, tp_dst: %s",
